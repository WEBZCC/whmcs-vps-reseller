...
{
    virtualizor_cloud_installpredefined();
    ...
    $configarray = array_merge($configarray, $config_array);
...
function virtualizor_cloud_CreateAccount($params)
{
    ...
    if ($loglevel > 0)
        logActivity('VIRT : ' . $virttype . ' - ' . $hvm . ' | ' . $Nvirt);
    if ($loglevel > 0)
        logActivity(var_export($params, 1));
    ...
    if (!empty($region) && $region != 'auto') {
        ...
        if ($loglevel > 1)
            logActivity('Region Chosen : ' . $sgid);
    }
    ...
    if ($loglevel > 1)
        logActivity('Return Values : ' . var_export($ret, 1));
    ...
    if (!empty($ret['newvs']['vpsid'])) {
        ...
        if ($loglevel > 0)
            logActivity('Did we found anything : ' . var_export($sel_res, 1));
...
function virtualizor_cloud_ChangePassword($params)
{
    ...
    if ($loglevel > 0)
        logActivity('Post Result : ' . var_export($ret, 1));
...
function virtualizor_cloud_ChangePackage($params)
{
    ...
    $post_vps = $data['vps'];
    if ($loglevel > 0)
        logActivity('Change Package Params : ' . var_export($params, 1));
    if ($loglevel > 0)
        logActivity('Orig VPS : ' . var_export($post_vps, 1));
